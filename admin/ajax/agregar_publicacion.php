<?php 
	...
	if(isset($_FILES) && isset($_FILES['imagefile']) && !empty($_FILES['imagefile']['name']) && !empty($_FILES['imagefile']['tmp_name'])){
		...
		$sql="INSERT INTO post (titulo, contenido, url_imagen, tipo, fecha) VALUES ('{$titulo}','{$contenido}','{$image_name}','{$tipo}', '{$fecha}')";
	}else{
		//echo 'No existe archivo para subir';
		$sql="INSERT INTO post (titulo, contenido, tipo, fecha) VALUES ('{$titulo}','{$contenido}','{$tipo}', '{$fecha}')";
	}
